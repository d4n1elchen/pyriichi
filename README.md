...
**注意**：輸入和輸出格式現在已統一為 `[5p]` 格式。為了向後兼容，舊的 `5p]` 格式仍然支持，但建議使用新的括號格式。
...
print(format_tiles(red_tiles))  # 輸出: [5p]6p7p（格式一致）
...
- **紅寶牌格式**：輸入和輸出統一使用 `[5p]` 格式（括號包裹），支持往返轉換。為了向後兼容，舊的 `5p]` 格式仍然支持
